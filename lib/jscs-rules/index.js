...
Object.keys(valueForRule).forEach(function (ruleName) {
  ...
  if (typeof ruleValue.forEach === 'function') {
    rules[ruleName] = new Rule(ruleName,
      new Rule.ArrSubsetRuleValues(ruleValue),
      Rule.concatReducer
    );
  } else if (typeof ruleValue === 'object') {
    rules[ruleName] = new Rule(ruleName,
      new Rule.HashSubsetRuleValues(ruleValue),
      Rule.mergeReducer
    );
  } else {
    rules[ruleName] = new Rule(ruleName,
  ...
  return new Promise(function (resolve, reject) {
    fs.readdir(base, function (err, files) {
      ...
      const availableRules = ruleNames.filter(function (ruleName) {
        ...
        if (rules[ruleName]) {
          curRules[ruleName] = rules[ruleName];
        } else {
          curRules[ruleName] = new Rule(ruleName,
            new Rule.BoolRuleValues(),
