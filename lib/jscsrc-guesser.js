  ...
  return getRules().then(function (rules) {
    ...
    const promises = ruleSets.map(function (ruleSet) {
      return runner(ruleSet, config, paths).then(function (result) {
        ...
        Object.keys(ruleSet).forEach(function (ruleName) {
          const ruleValue = {
            name: ruleName,
            value: ruleSet[ruleName]
          };
          if (result.indexOf(ruleName) === -1) {
            acceptedRuleValues.push(ruleValue);
          } else {
            rejectedRuleValues.push(ruleValue);
          }
        });
    ...
    return Promise.all(promises).then(function (results) {
      ...
      const groupedConfig = acceptedRuleValues.reduce(function (groups, ruleValue) {
        if (!groups[ruleValue.name]) {
          groups[ruleValue.name] = [];
        }
        groups[ruleValue.name].push(ruleValue.value);
        return groups;
      }, {});

      const config = Object.keys(groupedConfig).reduce(function (config, ruleName) {
        const filteredValues = groupedConfig[ruleName].filter(function (value) {
          return rules[ruleName].filterValue(value, groupedConfig);
        });
        if (filteredValues.length > 0) {
          config[ruleName] = rules[ruleName].reduceValues(filteredValues);
        }
        return config;
      }, {});
